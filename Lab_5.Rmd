...
To create a scatterplot in R you can use the function `plot()`. Firsts arguments of the function are the variables you want to vizualise and the other arguments are for aesthetics (title of the axis, colors etc.)
...
We can create a new column in the dataset called "res" where we extract the model residuals:
...
1. What is your dependent and independent variable in your analysis? Justify the answer.
2. What is the value of Pearson's r between your variables? What does that mean?
3. What value does your R square have? What does that mean?
4. Look at the scatter plot you made in point 2 and the res column in the dataset - are there any countries far away from the others? Which countries are these?
5. What is the value of the intercept? What does that mean?
6. What is the value of your regression coefficient? What does that mean?

# Submit 

- Use the document Submission template_exercise5.docx.
- Enter two scatter charts - one showing only the scatter plot with colors (but not regression line) and one showing the regression line and the scatter plot (but not country codes). The regression line and the points should have different colors. Place the charts side by side. Remember to change the axis titles to Norwegian if you are writing in Norwegian (you can do this in Variable View by clicking Label for your variable).
- Copy-paste tables showing the regression coefficients and R-square of Word (Step 5) - This will be in the summary of your **lm object**.
- Insert small chart and table texts that tell what the chart / table shows. You should be brief, but precise, and include all the information needed to understand what the table / chart shows. An example for Figure 3 from Exercise 1 is shown below.
- Answer the questions asked across this exercise.
...
- Up to 15 points are given for this exercise if submitted on time. There is no tests related to this specific exercise, but a test for LM5 where test questions from the practice tests are repeated.